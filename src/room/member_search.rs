...
pub fn search_room_members_streaming_with_sort(
    ...
    if search_text.is_empty() {
        let all_results: Vec<usize> = if let Some(ref sort_data) = precomputed_sort {
            ...
            valid_members.into_iter().map(|(_, _, idx)| idx).collect()
        };
    ...
    while sent_count < total_results {
        ...
        let batch: Vec<usize> = all_matches
            ...
            .map(|slice| slice.iter().map(|(_, idx)| *idx).collect())
...
fn match_member_with_priority(member: &RoomMember, search_text: &str) -> Option<u8> {
    ...
    if case_insensitive {
        if let Some(display) = display_name {
            if display.eq_ignore_ascii_case(search_text) {
                return Some(1);
            }
        }
    }
    ...
    if case_insensitive {
        if user_id.eq_ignore_ascii_case(search_text)
            || (!search_has_at
                && user_id.starts_with('@')
                && user_id
                    .strip_prefix('@')
                    .is_some_and(|id| id.eq_ignore_ascii_case(search_text)))
        {
            return Some(3);
        }
    }
    ...
    if case_insensitive {
        if let Some(display) = display_name {
            if starts_with_ignore_ascii_case(display, search_text) {
                return Some(5);
            }
        }
    }
    ...
    if user_id.starts_with(search_text)
        || (!search_has_at
            && user_id.starts_with('@')
            && user_id
                .strip_prefix('@')
                .is_some_and(|id| id.starts_with(search_text)))
    ...
    if case_insensitive {
        if starts_with_ignore_ascii_case(user_id, search_text)
            || (!search_has_at
                && user_id.starts_with('@')
                && user_id
                    .strip_prefix('@')
                    .is_some_and(|id| starts_with_ignore_ascii_case(id, search_text)))
        {
            return Some(7);
        }
    }
    ...
    if let Some(display) = display_name {
        ...
        if case_insensitive && contains_ignore_ascii_case(display, search_text) {
            return Some(8);
        }
    }
    ...
    if case_insensitive {
        if contains_ignore_ascii_case(user_id, search_text)
            || contains_ignore_ascii_case(localpart, search_text)
        {
            return Some(9);
        }
    } else if user_id.contains(search_text) || localpart.contains(search_text) {
        return Some(9);
    }
