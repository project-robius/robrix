...
pub fn text_preview_of_other_state(
    ...
    let text = match other_state.content() {
        ...
        AnyOtherStateEventContentChange::RoomCanonicalAlias(StateEventContentChange::Original { content, .. }) => {
            ...
            let safe_alias = if format_as_html {
                Cow::Owned(htmlize::escape_text(alias).into_owned())
            } else {
                Cow::Borrowed(alias)
            };
            Some(format!("set the main address of this room to {safe_alias}."))
        }
        ...
        AnyOtherStateEventContentChange::RoomGuestAccess(StateEventContentChange::Original { content, .. }) => {
            Some(match &content.guest_access {
                ...
                custom => {
                    let safe_custom = if format_as_html {
                        Cow::Owned(htmlize::escape_text(custom.as_str()).into_owned())
                    } else {
                        Cow::Borrowed(custom.as_str())
                    };
                    format!("has set custom guest access rules for this room: {safe_custom}")
                }
            })
        }
        ...
        AnyOtherStateEventContentChange::RoomJoinRules(StateEventContentChange::Original { content, .. }) => {
            Some(match &content.join_rule {
                ...
                custom => {
                    ...
                    format!("set custom join rules for this room: {safe_custom}")
                }
            })
        }
        ...
        AnyOtherStateEventContentChange::RoomTombstone(StateEventContentChange::Original { content, .. }) => {
            ...
            let safe_uri = if format_as_html {
                Cow::Owned(htmlize::escape_text(&uri).into_owned())
            } else {
                Cow::Borrowed(uri.as_str())
            };
            Some(format!("closed this room and upgraded it to {safe_uri}"))
        }
