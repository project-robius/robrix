...
use matrix_sdk_ui::{
    ...
    timeline::{
        AnyOtherFullStateEventContent, EventTimelineItem, LiveBackPaginationStatus, RepliedToInfo,
...
use crate::{
    ...
    home::{
        ...
        rooms_list::{self, RoomPreviewAvatar, RoomPreviewEntry, RoomsListUpdate},
        rooms_sidebar::apply_all_rooms_count,
    },
...
async fn login(cli: Cli) -> Result<(Client, Option<String>)> {
    ...
    if client.logged_in() {
        ...
        rooms_list::enqueue_people_list_update(RoomsListUpdate::Status {
            status: format!("Logged in as {}. Loading rooms...", &cli.username),
        });
        ...
        match request {
            MatrixRequest::PaginateRoomTimeline {
                ...
                let timeline = {
                    ...
                    let should_spawn_pagination_status_task =
                        match room_info.pagination_status_task.as_ref() {
...
async fn async_main_loop() -> Result<()> {
    ...
    let Some(cli) = cli else {
        rooms_list::enqueue_rooms_list_update(RoomsListUpdate::Status {
            status: String::from(
                "Error: missing username and password in 'login.toml' file. \
    ...
    let specified_username: Option<OwnedUserId> =
        cli.username.to_string().try_into().ok().or_else(|| {
            let homeserver_url = cli.homeserver.as_deref().unwrap_or("matrix.org");
    ...
    let (client, _sync_token) =
        if let Ok(restored) = persistent_state::restore_session(specified_username).await {
            restored
        } else {
    ...
    log!(
        "Populating initial set of {} known rooms.",
        all_known_rooms.len()
    );
        ...
        for diff in batch {
            match diff {
                ...
                VectorDiff::Clear => {
                    ...
                    rooms_list::enqueue_rooms_list_update(RoomsListUpdate::ClearRooms);
                    rooms_list::enqueue_people_list_update(RoomsListUpdate::ClearRooms);
                }
                ...
                VectorDiff::Reset { values } => {
                    ...
                    if LOG_ROOM_LIST_DIFFS {
                        log!("room_list: diff Reset, new length {}", values.len());
                    }
...
async fn remove_room(room: room_list_service::Room) {
    ALL_ROOM_INFO.lock().unwrap().remove(room.room_id());

    if room.is_direct().await.unwrap_or(true) {
        rooms_list::enqueue_people_list_update(RoomsListUpdate::RemoveRoom(
            room.room_id().to_owned(),
        ))
    } else {
        rooms_list::enqueue_rooms_list_update(RoomsListUpdate::RemoveRoom(
            room.room_id().to_owned(),
        ))
    }
...
async fn add_new_room(room: &room_list_service::Room) -> Result<()> {
    ...
    log!(
        "Adding new room: {:?}, room_id: {room_id}",
    ...
    if room.is_direct().await.unwrap_or(true) {
        rooms_list::enqueue_people_list_update(RoomsListUpdate::AddRoom(RoomPreviewEntry {
            room_id: room_id.clone(),
            latest,
    ...
    while let Some(batch) = subscriber.next().await {
        ...
        if num_updates > 0 {
            ...
            if let Some(new_latest) = new_latest_event {
                ...
                {
                    let room_avatar_changed = update_latest_event(&room_id, &new_latest).await;
...
async fn update_latest_event(room_id: &RoomId, event_tl_item: &EventTimelineItem) -> bool {
    let mut room_avatar_changed = false;
    let room = get_client().unwrap().get_room(room_id).unwrap();
    ...
    let latest_message_text =
        text_preview_of_timeline_item(event_tl_item.content(), latest_event_sender_username)
    ...
    match event_tl_item.content() {
        TimelineItemContent::OtherState(other) => match other.content() {
            AnyOtherFullStateEventContent::RoomName(FullStateEventContent::Original {
                ...
                if room.is_direct().await.unwrap_or(true) {
                    rooms_list::enqueue_people_list_update(RoomsListUpdate::UpdateRoomName {
                        room_id: room_id.to_owned(),
    ...
    if room.is_direct().await.unwrap_or(true) {
        rooms_list::enqueue_people_list_update(RoomsListUpdate::UpdateLatestEvent {
            room_id: room_id.to_owned(),
...
fn spawn_fetch_room_avatar(room: Room) {
    ...
    Handle::current().spawn(async move {
        ...
        if room.is_direct().await.unwrap_or(true) {
            rooms_list::enqueue_people_list_update(RoomsListUpdate::UpdateRoomAvatar {
                room_id,
                avatar,
            })
        } else {
