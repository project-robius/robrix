                ...
                Handle::current().spawn(async move {
                    match client.send(Request::new(search_categories)).await {
                        Ok(response) => {
                            ...
                            for item in response.search_categories.room_events.results.iter().map(|f| f.result.clone().and_then(|f|f.deserialize().ok())).filter(|f| f.is_some()).map(|f| f.unwrap()) {
                                timeline_events.push_back(item);
                            }
                            match sender.send(TimelineUpdate::SearchResult(timeline_events)) {
                                Ok(_v) => {
                                    SignalToUI::set_ui_signal();
                                }
                                Err(e) => {}
