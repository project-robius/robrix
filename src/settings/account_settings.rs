...
use crate::{avatar_cache, logout::logout_confirm_modal::{LogoutAction, LogoutConfirmModalAction}, profile::user_profile::UserProfile, shared::{avatar::{AvatarState, AvatarWidgetExt}, popup_list::{enqueue_popup_notification, PopupItem, PopupKind}, styles::*}, sliding_sync::{AccountDataAction, MatrixRequest, submit_async_request}, utils};
...
impl MatchEvent for AccountSettings {
    ...
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions) {
        ...
        for action in actions {
            if let Some(AccountDataAction::DisplayNameChangeFailed(_)) = action.downcast_ref() {
