...
use ruma::{events::{relation::InReplyTo, room::message::{EmoteMessageEventContent, FormattedBody, MessageFormat, MessageType, NoticeMessageEventContent, Relation, RoomMessageEventContent, TextMessageEventContent}, sticker::StickerEventContent, AnyMessageLikeEventContent, AnySyncTimelineEvent, AnyTimelineEvent}, uint, OwnedRoomId};
...
pub fn populate_message_view(
    ...
    let use_compact_view = match prev_event {
        Some(AnyTimelineEvent::MessageLike(prev_event_tl_item)) => match prev_event_tl_item.original_content() {
            Some(AnyMessageLikeEventContent::RoomMessage(_)) | Some(AnyMessageLikeEventContent::Sticker(_)) => {
                ...
                println!("relations {:?}", prev_event_tl_item.relations());
    ...
    let mut message_abilities = MessageAbilities::empty();
...
pub fn draw_walk(room_screen: &mut RoomScreen, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
            ...
            let item = {
                ...
                let (item, item_new_draw_status) = {
                    ...
                    match current_item {
                        ...
                        AnyTimelineEvent::State(_) => continue
                    }
                };
