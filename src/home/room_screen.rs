...
use matrix_sdk::{
    ruma::{
        ...
        matrix_uri::MatrixId,
        uint, MatrixToUri, MatrixUri, MilliSecondsSinceUnixEpoch, OwnedRoomId, RoomId,
    },
    OwnedServerName,
};
use matrix_sdk_ui::timeline::{
    self, AnyOtherFullStateEventContent, EventTimelineItem, MemberProfileChange, MembershipChange,
    ReactionsByKeyBySender, RepliedToInfo, ReplyContent, RoomMembershipChange, TimelineDetails,
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Actions(actions) = event {
            for action in actions {
                ...
                if let TimelineAction::MessageReply(replied_to_info) =
                    action.as_widget_action().cast()
                {
                    let replying_preview_view = self.view(id!(replying_preview));

                    if let ReplyContent::Message(message) = replied_to_info.content() {
                        const MAX_REPLYING_PREVIEW_BODY_LENGTH: usize = 100;

                        // TODO: Get pretty name and picture.
                        let replying_preview_username = replied_to_info.sender();
                        ...
                        // TODO: Draw the body as MD
                        replying_preview_view
                            .html_or_plaintext(id!(body))
                            .show_plaintext(replying_preview_body);
                    }
...
impl Timeline {
    ...
    fn show_timeline(&mut self) {
        ...
        let (tl_state, first_time_showing_room) =
...
impl TimelineRef {
    ...
    fn set_room(&self, room_id: OwnedRoomId) {
        let Some(mut timeline) = self.borrow_mut() else {
            return;
        };
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Actions(actions) = event {
            for action in actions {
                ...
                if let MessageAction::MessageReply(item_id) = action.as_widget_action().cast() {
                    ...
                    if let Some(tl_item) = tl.items.get(tl_idx) {
                        if let Some(tl_event_item) = tl_item.as_event() {
                            ...
                            match tl_event_item.replied_to_info() {
                                Ok(replied_to_info) => {
                                    cx.widget_action(
                                        widget_uid,
                                        &scope.path,
                                        TimelineAction::MessageReply(replied_to_info),
                                    );
                                }
                                Err(e) => error!("{e}"),
                            }
            ...
            while let Ok(update) = tl.update_receiver.try_recv() {
                match update {
                    TimelineUpdate::NewItems {
                        ...
                        if let Some(top_event_id) =
                            tl.items.get(orig_first_id).map(|item| item.unique_id())
                        {
...
// TODO: Maybe populate from inside the Message widget?
    ...
    let (item, used_cached_item) = match message.msgtype() {
        MessageType::Text(text) => {
            ...
            if existed && item_drawn_status.content_drawn {
                ...
                if let Some(formatted_body) = text
                    .formatted
                    .as_ref()
                ...
                if let Some(in_reply_to_details) = message.in_reply_to() {
                    let reply_preview = item.view(id!(reply_preview));
                    reply_preview.set_visible(true);

                    match &in_reply_to_details.event {
                        TimelineDetails::Ready(details) => {
                            // TODO: Set user picture and pretty body
                            let in_reply_to_username = details.as_ref().sender();
                            reply_preview
                                .label(id!(username))
                            ...
                            //     // TODO: Maybe handle rest of the types of content
                            //     _ => "",
                            // };

                            // reply_preview
                            //     .html_or_plaintext(id!(body))
                            //     .set_text(in_reply_to_body);
                        }
                        _ => {}
                    }
                }
                    ...
                    MediaSource::Plain(mxc_uri) => {
                        ...
                        match media_cache.try_get_media_or_fetch(mxc_uri.clone(), None) {
                            ...
                            MediaCacheEntry::Requested => {
                                text_or_image_ref
                                    .set_text(&format!("Fetching image from {:?}", mxc_uri));
                            }
            ...
            if let Some(redacted_msg) = event_tl_item.latest_json() {
                if let Ok(old) = redacted_msg.deserialize() {
                    if let AnySyncTimelineEvent::MessageLike(
                        AnySyncMessageLikeEvent::RoomMessage(SyncMessageLikeEvent::Redacted(
                            redaction,
                        )),
                    ) = old
                    {
    ...
    fn populate_item_content(
        ...
        let text = match self.content() {
            ...
            AnyOtherFullStateEventContent::RoomGuestAccess(FullStateEventContent::Original {
                ...
                GuestAccess::Forbidden | _ => {
                    format!("has forbidden guests from joining this room.")
                }
            }),
            ...
            AnyOtherFullStateEventContent::RoomPowerLevels(_) => None,
...
impl SmallStateEventContent for MemberProfileChange {
    fn populate_item_content(
        ...
        item.label(id!(content))
            .set_text(&format!("{}{}.", name_text, avatar_text));
        new_drawn_status.content_drawn = true;
        (item, new_drawn_status)
    }
}
...
impl SmallStateEventContent for RoomMembershipChange {
    fn populate_item_content(
        ...
        let text = match self.change() {
            None | Some(MembershipChange::NotImplemented) | Some(MembershipChange::None) => {
            ...
            Some(MembershipChange::Error) => format!("had a membership change error."),
...
fn populate_small_state_event(
    ...
    let username = username_opt.unwrap_or_else(|| {
        ...
        let (username, profile_drawn) =
            set_avatar_and_get_username(cx, item.avatar(id!(avatar)), room_id, event_tl_item);
