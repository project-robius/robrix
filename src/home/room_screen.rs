...
pub struct Timeline {
    ...
    #[rust] update_receiver: Option<crossbeam_channel::Receiver<TimelineUpdate>>,
}

impl Drop for Timeline {
    fn drop(&mut self) {
        println!("$$$$ Timeline::drop() for room {:?}", self.room_id);
    }
}
...
impl Widget for Timeline {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Signal = event {
            ...
            while let Some(update) = self.update_receiver.as_mut().and_then(|r| r.try_recv().ok()) {
                match update { 
                    ...
                    TimelineUpdate::TimelineStartReached => {
                        println!("Timeline::handle_event(): timeline start reached for room {}", self.room_id.as_ref().unwrap());
                        self.fully_paginated = true;
                        done_loading = true;
                    }
            ...
            if done_loading {
                println!("TODO: hide topspace loading animation for room {}", self.room_id.as_ref().unwrap());
