    ...
    ruma::{
        ...
        events::{
            ...
            room::{
                ...
                message::{
                    AudioMessageEventContent, CustomEventContent, EmoteMessageEventContent,
                    FileMessageEventContent, FormattedBody, ImageMessageEventContent,
                    KeyVerificationRequestEventContent, LocationMessageEventContent, MessageFormat,
...
use matrix_sdk_ui::timeline::{
    self, EventTimelineItem, InReplyToDetails, MemberProfileChange, RepliedToInfo,
...
use crate::{
    avatar_cache,
    event_preview::{
        body_of_timeline_item, text_preview_of_member_profile_change, text_preview_of_other_state,
    ...
    location::{
        LocationAction, LocationRequest, LocationUpdate, get_latest_location,
        init_location_subscriber, request_location_update,
    },
    ...
    shared::{
        ...
        mentionable_text_input::MentionableTextInputWidgetRefExt,
    ...
    sliding_sync::{
        self, BackwardsPaginateUntilEventRequest, MatrixRequest, PaginationDirection,
        TimelineRequestSender, UserPowerLevels, get_client, submit_async_request,
        take_timeline_endpoints,
    },
    utils::{
        self, ImageFormat, MEDIA_THUMBNAIL_FORMAT, MediaFormatConst, unix_time_millis_to_datetime,
    },
};
use rangemap::RangeSet;

use super::{
    editing_pane::{EditingPaneAction, EditingPaneWidgetExt},
    event_reaction_list::ReactionData,
    loading_pane::LoadingPaneRef,
    new_message_context_menu::{MessageAbilities, MessageDetails},
    room_read_receipt::{self, MAX_VISIBLE_AVATARS_IN_READ_RECEIPT, populate_read_receipts},
};
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Actions(actions) = event {
            for (_, wr) in portal_list.items_with_actions(actions) {
                ...
                } = reaction_list.hover_in(actions)
                {
                    ...
                    cx.widget_action(self.widget_uid(), &scope.path, TooltipAction::HoverIn {
                        widget_rect,
                        text: tooltip_text,
                        text_color: None,
                        bg_color,
            ...
            */
            ...
            if self.button(id!(location_button)).clicked(actions) {
                ...
                if let Err(_e) = init_location_subscriber(cx) {
                    ...
                    enqueue_popup_notification(String::from(
        ...
        if !is_pane_shown || !is_interactive_hit {
            ...
            let mut actions_generated_within_this_room_screen =
                cx.capture_actions(|cx| self.view.handle_event(cx, event, scope));
...
impl RoomScreen {
            ...
            match update {
                ...
                TimelineUpdate::NewItems { new_items, changed_indices, is_append, clear_cache } => {
                    ...
                    //
                        ...
                        warning!(
                            "!!! Couldn't find new event with matching ID for ANY event currently visible in the portal list"
                        );
                    }
                    ...
                    if clear_cache {
                        ...
                        } = &mut loading_pane_state
                        {
                            *events_paginated += new_items.len().saturating_sub(tl.items.len());
                            log!(
                                "While finding target event {target_event_id}, loaded {events_paginated} messages..."
                            );
                ...
                TimelineUpdate::MessageEdited { timeline_event_id, result } => {
                    self.view.editing_pane(id!(editing_pane)).handle_edit_result(
                        cx,
                        timeline_event_id,
                        result,
                    );
                ...
                TimelineUpdate::UserPowerLevels(user_power_level) => {
                    ...
                    self.view.view(id!(input_bar)).set_visible(cx, can_send_message);
                    self.view
    ...
    fn handle_link_clicked(
        &mut self, cx: &mut Cx, action: &Action, pane: &UserProfileSlidingPaneRef,
    ) -> bool {
            ...
            let mut handle_uri = |id: &MatrixId, _via: &[OwnedServerName]| -> bool {
                match id {
                    ...
                    MatrixId::User(user_id) => {
                        ...
                        self.show_user_profile(cx, pane, UserProfilePaneInfo {
    ...
    fn handle_message_actions(
        &mut self, cx: &mut Cx, actions: &ActionsBuf, portal_list: &PortalListRef,
        loading_pane: &LoadingPaneRef,
    ) {
        let room_screen_widget_uid = self.widget_uid();
        for action in actions {
            match action.as_widget_action().widget_uid_eq(room_screen_widget_uid).cast() {
                ...
                MessageAction::Reply(details) => {
                    let mut success = false;
                    if let Some(event_tl_item) = self
                        .tl_state
                        .as_ref()
                    ...
                    if !success {
                        enqueue_popup_notification(
                            "Could not find message in timeline to reply to.".to_string(),
                        );
                        error!(
                ...
                MessageAction::Edit(details) => {
                        ...
                        .filter(|ev| ev.event_id() == details.event_id.as_deref())
                    {
                        ...
                        enqueue_popup_notification(
                            "Could not find message in timeline to edit.".to_string(),
                        );
                        error!(
                ...
                MessageAction::Pin(_details) => {
                    // TODO
                    enqueue_popup_notification(
                        "Pinning messages is not yet implemented.".to_string(),
                    );
                },
                MessageAction::Unpin(_details) => {
                    // TODO
                    enqueue_popup_notification(
                        "Unpinning messages is not yet implemented.".to_string(),
                    );
                },
                MessageAction::CopyText(details) => {
                        ...
                        .and_then(|tl_item| tl_item.as_event().map(body_of_timeline_item))
                    {
                        ...
                        enqueue_popup_notification(
                            "Could not find message in timeline to copy text from.".to_string(),
                        );
                        error!(
                ...
                MessageAction::CopyHtml(details) => {
                    ...
                    if !success {
                        enqueue_popup_notification(
                            "Could not find message in timeline to copy HTML from.".to_string(),
                        );
                        error!(
                ...
                MessageAction::CopyLink(details) => {
                    ...
                    if let Some(event_id) = details.event_id {
                        ...
                        enqueue_popup_notification(
                            "Couldn't create permalink to message.".to_string(),
                        );
                        error!(
                ...
                MessageAction::ViewSource(_details) => {
                    enqueue_popup_notification(
                        "Viewing an event's source is not yet implemented.".to_string(),
                    );
                ...
                MessageAction::JumpToRelated(details) => {
                    ...
                    if let Some(index) = related_msg_tl_index {
                        ...
                        loading_pane.set_state(cx, LoadingPaneState::BackwardsPaginateUntilEvent {
                ...
                MessageAction::Redact { details, reason } => {
                    ...
                    if !success {
                        enqueue_popup_notification(
                            "Couldn't find message in timeline to delete.".to_string(),
                        );
                        error!(
    ...
    fn show_user_profile(
        &mut self, cx: &mut Cx, pane: &UserProfileSlidingPaneRef, info: UserProfilePaneInfo,
    ) {
    ...
    fn show_editing_pane(
        &mut self, cx: &mut Cx, event_tl_item: EventTimelineItem, room_id: OwnedRoomId,
    ) {
        ...
        self.editing_pane(id!(editing_pane)).show(cx, event_tl_item, room_id);
        self.redraw(cx);
    }
    ...
    fn show_timeline(&mut self, cx: &mut Cx) {
        let room_id =
            self.room_id.clone().expect("BUG: Timeline::show_timeline(): no room_id was set.");
    ...
    fn restore_state(&mut self, cx: &mut Cx, tl_state: &mut TimelineUiState) {
        let SavedState { first_index_and_scroll, message_input_state, replying_to, editing_event } =
    ...
    pub fn set_displayed_room(&mut self, cx: &mut Cx, room_id: OwnedRoomId, room_name: String) {
    ...
    fn send_user_read_receipts_based_on_scroll_pos(
        &mut self, _cx: &mut Cx, actions: &ActionsBuf, portal_list: &PortalListRef,
    ) {
        //stopped scrolling
        if portal_list.scrolled(actions) {
            return;
            ...
            if *index != first_index {
                if first_index >= *index {
                    ...
                    if tl_state.scrolled_past_read_marker {
                        ...
                        if let Some(own_user_receipt_timestamp) =
                            &tl_state.latest_own_user_receipt.clone().and_then(|receipt| receipt.ts)
                        {
...
impl RoomScreenRef {
    ...
    pub fn set_displayed_room(&self, cx: &mut Cx, room_id: OwnedRoomId, room_name: String) {
...
fn find_new_item_matching_current_item(
    cx: &mut Cx, portal_list: &PortalListRef, starting_at_curr_idx: usize,
    curr_items: &Vector<Arc<TimelineItem>>, new_items: &Vector<Arc<TimelineItem>>,
        ...
        match self {
            Self::Message(msg) => match msg.msgtype() {
                ...
                MessageType::ServerNotice(server_notice) => {
                    MessageOrStickerType::ServerNotice(server_notice)
                },
                ...
                MessageType::VerificationRequest(verification_request) => {
                    MessageOrStickerType::VerificationRequest(verification_request)
                },
...
impl MessageOrStickerType<'_> {
    ...
    pub fn get_image_info(&self) -> Option<(Option<ImageInfo>, MediaSource)> {
        match self {
            Self::Image(image) => {
                Some((image.info.clone().map(|info| *info), image.source.clone()))
            },
            Self::Sticker(sticker) => {
                Some((Some(sticker.info.clone()), sticker.source.clone().into()))
            },
            _ => None,
        }
    }
...
fn populate_message_view(
    cx: &mut Cx2d, list: &mut PortalList, item_id: usize, room_id: &OwnedRoomId,
    event_tl_item: &EventTimelineItem, message: MessageOrSticker,
    prev_event: Option<&Arc<TimelineItem>>, media_cache: &mut MediaCache,
    user_power_levels: &UserPowerLevels, item_drawn_status: ItemDrawnStatus,
    ...
    let (item, used_cached_item) = match message.get_type() {
        MessageOrStickerType::Text(TextMessageEventContent { body, formatted, .. }) => {
            ...
            let template =
                if use_compact_view { live_id!(CondensedMessage) } else { live_id!(Message) };
        ...
        MessageOrStickerType::ServerNotice(sn) => {
            ...
            if existed && item_drawn_status.content_drawn {
                ...
                populate_text_message_content(
                    ...
                    &sn.body,
                    Some(&FormattedBody { format: MessageFormat::Html, body: formatted }),
                );
                new_drawn_status.content_drawn = true;
                (item, false)
            }
        ...
        mtype @ MessageOrStickerType::Image(_) | mtype @ MessageOrStickerType::Sticker(_) => {
            has_html_body = match mtype {
                MessageOrStickerType::Image(image) => {
                    image.formatted.as_ref().is_some_and(|f| f.format == MessageFormat::Html)
                },
                _ => false,
            };
        ...
        MessageOrStickerType::File(file_content) => {
            has_html_body =
                file_content.formatted.as_ref().is_some_and(|f| f.format == MessageFormat::Html);
            let template =
                if use_compact_view { live_id!(CondensedMessage) } else { live_id!(Message) };
            let (item, existed) = list.item_with_existed(cx, item_id, template);
            if existed && item_drawn_status.content_drawn {
        ...
        MessageOrStickerType::Audio(audio) => {
            has_html_body =
                audio.formatted.as_ref().is_some_and(|f| f.format == MessageFormat::Html);
            let template =
                if use_compact_view { live_id!(CondensedMessage) } else { live_id!(Message) };
            let (item, existed) = list.item_with_existed(cx, item_id, template);
            if existed && item_drawn_status.content_drawn {
        ...
        MessageOrStickerType::Video(video) => {
            has_html_body =
                video.formatted.as_ref().is_some_and(|f| f.format == MessageFormat::Html);
            let template =
                if use_compact_view { live_id!(CondensedMessage) } else { live_id!(Message) };
            let (item, existed) = list.item_with_existed(cx, item_id, template);
            if existed && item_drawn_status.content_drawn {
        ...
        MessageOrStickerType::VerificationRequest(verification) => {
            has_html_body =
                verification.formatted.as_ref().is_some_and(|f| f.format == MessageFormat::Html);
    ...
    if let Some(dt) = unix_time_millis_to_datetime(&ts_millis) {
        ...
        if !use_compact_view {
            item.label(id!(profile.datestamp)).set_text(cx, &format!("{}", dt.date_naive()));
        }
    } else {
        item.label(id!(profile.timestamp)).set_text(cx, &format!("{}", ts_millis.get()));
    }
...
fn does_message_mention_current_user(message: &MessageOrSticker) -> bool {
    ...
    match message {
        ...
        MessageOrSticker::Message(msg) => msg
            .mentions()
            .is_some_and(|mentions| mentions.room || mentions.user_ids.contains(&current_user_id)),
...
fn populate_text_message_content(
    cx: &mut Cx, message_content_widget: &HtmlOrPlaintextRef, body: &str,
    formatted_body: Option<&FormattedBody>,
) {
    // The message was HTML-formatted rich text.
    if let Some(fb) =
        formatted_body.as_ref().and_then(|fb| (fb.format == MessageFormat::Html).then_some(fb))
    {
        message_content_widget
            .show_html(cx, utils::linkify(utils::trim_start_html_whitespace(&fb.body), true));
    }
...
fn populate_image_message_content(
    cx: &mut Cx2d, text_or_image_ref: &TextOrImageRef,
    image_info_source: Option<(Option<ImageInfo>, MediaSource)>, body: &str,
    ...
    let (mimetype, _width, _height) = image_info_source
        .as_ref()
        .and_then(|(info, _)| {
            info.as_ref().map(|info| (info.mimetype.as_deref(), info.width, info.height))
        })
        .unwrap_or_default();
    ...
    let mut fetch_and_show_image_uri =
        |cx: &mut Cx2d, mxc_uri: OwnedMxcUri, image_info: Option<&ImageInfo>| {
            match media_cache
                .try_get_media_or_fetch(mxc_uri.clone(), Some(MEDIA_THUMBNAIL_FORMAT.into()))
            {
                ...
                MediaCacheEntry::Requested => {
                    if let Some(image_info) = image_info {
                        if let (Some(ref blurhash), Some(width), Some(height)) =
                            (image_info.blurhash.clone(), image_info.width, image_info.height)
                        {
                            let show_image_result = text_or_image_ref.show_image(cx, |cx, img| {
    ...
    let mut fetch_and_show_media_source =
        |cx: &mut Cx2d, media_source: MediaSource, image_info: Option<&ImageInfo>| {
            match media_source {
...
fn populate_file_message_content(
    cx: &mut Cx, message_content_widget: &HtmlOrPlaintextRef,
...
fn populate_audio_message_content(
    cx: &mut Cx, message_content_widget: &HtmlOrPlaintextRef, audio: &AudioMessageEventContent,
) -> bool {
    // Display the file name, human-readable size, caption, and a button to download it.
    ...
    let (duration, mime, size) = audio
        ...
        .map(|info| {
            (
                info.duration.map(|d| format!("  {:.2} sec,", d.as_secs_f64())).unwrap_or_default(),
                info.mimetype.as_ref().map(|m| format!("  {m},")).unwrap_or_default(),
                info.size
                    .map(|bytes| format!("  ({}),", ByteSize::b(bytes.into())))
                    .unwrap_or_default(),
            )
...
fn populate_video_message_content(
    cx: &mut Cx, message_content_widget: &HtmlOrPlaintextRef, video: &VideoMessageEventContent,
) -> bool {
    // Display the file name, human-readable size, caption, and a button to download it.
...
fn draw_replied_to_message(
    cx: &mut Cx2d, replied_to_message_view: &ViewRef, room_id: &OwnedRoomId,
    in_reply_to: Option<&InReplyToDetails>, message_event_id: Option<&EventId>,
...
fn populate_preview_of_timeline_item(
    cx: &mut Cx, widget_out: &HtmlOrPlaintextRef, timeline_item_content: &TimelineItemContent,
    sender_username: &str,
) {
...
trait SmallStateEventContent {
    ...
    fn populate_item_content(
        &self, cx: &mut Cx, list: &mut PortalList, item_id: usize, item: WidgetRef,
        event_tl_item: &EventTimelineItem, username: &str, item_drawn_status: ItemDrawnStatus,
...
impl SmallStateEventContent for RedactedMessageEventMarker {
    fn populate_item_content(
        &self, cx: &mut Cx, _list: &mut PortalList, _item_id: usize, item: WidgetRef,
        event_tl_item: &EventTimelineItem, original_sender: &str,
        _item_drawn_status: ItemDrawnStatus, mut new_drawn_status: ItemDrawnStatus,
    ) -> (WidgetRef, ItemDrawnStatus) {
...
impl SmallStateEventContent for timeline::OtherState {
    fn populate_item_content(
        &self, cx: &mut Cx, list: &mut PortalList, item_id: usize, item: WidgetRef,
        _event_tl_item: &EventTimelineItem, username: &str, _item_drawn_status: ItemDrawnStatus,
        mut new_drawn_status: ItemDrawnStatus,
    ) -> (WidgetRef, ItemDrawnStatus) {
        let item = if let Some(text_preview) = text_preview_of_other_state(self) {
            item.label(id!(content)).set_text(cx, &text_preview.format_with(username));
            new_drawn_status.content_drawn = true;
            item
        } else {
...
impl SmallStateEventContent for RoomMembershipChange {
    fn populate_item_content(
        ...
        let Some(preview) = text_preview_of_room_membership_change(self) else {
            ...
            return (list.item(cx, item_id, live_id!(Empty)), ItemDrawnStatus::new());
        };

        item.label(id!(content)).set_text(cx, &preview.format_with(username));
        new_drawn_status.content_drawn = true;
        (item, new_drawn_status)
    }
}
...
fn populate_small_state_event(
    cx: &mut Cx, list: &mut PortalList, item_id: usize, room_id: &OwnedRoomId,
    event_tl_item: &EventTimelineItem, event_content: &impl SmallStateEventContent,
    ...
    let username_opt =
        skip_redrawing_profile.then(|| get_profile_display_name(event_tl_item)).flatten();
    ...
    let username = username_opt.unwrap_or_else(|| {
        ...
        set_timestamp(cx, &item, id!(left_container.timestamp), event_tl_item.timestamp());
        new_drawn_status.profile_drawn = profile_drawn;
        username
    });
...
fn set_timestamp(
    cx: &mut Cx, item: &WidgetRef, live_id_path: &[LiveId], timestamp: MilliSecondsSinceUnixEpoch,
) {
    if let Some(dt) = unix_time_millis_to_datetime(&timestamp) {
        // format as AM/PM 12-hour time
        item.label(live_id_path).set_text(cx, &format!("{}", dt.time().format("%l:%M %P")));
    } else {
        item.label(live_id_path).set_text(cx, &format!("{}", timestamp.get()));
    }
}
...
impl LocationPreview {
    ...
    pub fn get_current_data(&self) -> Option<(Coordinates, Option<SystemTime>)> {
        self.coords.as_ref().and_then(|res| res.ok()).map(|c| (c, self.timestamp))
    }
}
...
pub struct Message {
    ...
    #[rust]
    long_press_state: LongPressState,
        ...
        match hit {
            ...
            Hit::FingerUp(fe) if fe.is_over && fe.was_tap() => {
                ...
                if fe.is_primary_hit()
                    && self.view(id!(replied_to_message)).area().rect(cx).contains(fe.abs)
                {
                    cx.widget_action(
                        details.room_screen_widget_uid,
                        &scope.path,
