...
use crate::{home::room_read_receipt::{AvatarRowWidgetRefExt, MAX_VISIBLE_AVATARS_IN_READ_RECEIPT}, sliding_sync::TimelineKind};
...
impl SmallStateGroupManager {
    ...
    fn create_room_creation_group(&mut self, events: &[UserEvent]) {
        ...
        let start_index = first_event.index;
        let end_index = last_event.index + 1;
        ...
        let header_event_id = match first_event.event_id.clone() {
            Some(id) => id,
            None => {
                // Create synthetic event ID based on range
                let Ok(id) = OwnedEventId::try_from(format!("$synthetic_{}_{}", start_index, end_index).as_str()) else {
                    return;
                };
                id
            }
        ...
        match self.groups_by_event_id.entry(header_event_id) {
            Entry::Occupied(mut entry) => {
                ...
                group.room_creator = first_event.sender.clone();
...
pub fn populate_avatar_row_from_user_ids(
    ...
    avatar_row.avatar_row(cx, ids!(user_event_avatar_row)).set_avatar_row(cx, timeline_kind, None, &receipts_map);
}
