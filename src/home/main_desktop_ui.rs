        ...
        for action in actions {
            ...
            match action.downcast_ref() {
                Some(MainDesktopUiAction::LoadDockFromAppState) => {
                    let app_state = scope.data.get_mut::<AppState>().unwrap();
                    let dock = self.view.dock(id!(dock));
                    self.room_order = app_state.save_dock_state.room_order.clone();
                    self.open_rooms = app_state.save_dock_state.open_rooms.clone();
                    if app_state.save_dock_state.dock_items.is_empty() {
                        return;
                    }

                    if let Some(mut dock) = dock.borrow_mut() {
                        dock.load_state(cx, app_state.save_dock_state.dock_items.clone());
                        for (head_live_id, (_, widget)) in dock.items().iter() {
                            match app_state.save_dock_state.open_rooms.get(head_live_id) {
                ...
                Some(MainDesktopUiAction::SaveDockIntoAppState) => {
                    ...
                    if let Some(dock_items) = dock.clone_state() {
                        app_state.save_dock_state.dock_items = dock_items;
                    }
                    app_state.save_dock_state.open_rooms = self.open_rooms.clone();
                    app_state.save_dock_state.room_order = self.room_order.clone();
                }
                _ => {}
            }
        }
