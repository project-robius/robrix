...
impl CreateBotModal {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions) {
        let cancel_button = self.view.button(cx, ids!(card.buttons.cancel_button));
        let create_button = self.view.button(cx, ids!(card.buttons.create_button));
        let username_input = self.view.text_input(cx, ids!(card.form.username_input));
        let display_name_input = self.view.text_input(cx, ids!(card.form.display_name_input));
        let prompt_input = self.view.text_input(cx, ids!(card.form.prompt_input));
        let mut status_label = self.view.label(cx, ids!(card.status_label));

        let dismissed = actions
            .iter()
            .any(|a| matches!(a.downcast_ref(), Some(ModalAction::Dismissed)));
        if cancel_button.clicked(actions) || dismissed {
            // If the modal was dismissed by clicking outside of it, do not re-emit
            // our own Close action or we will feed the outer Modal another close request.
            if !dismissed {
    ...
    pub fn show(&mut self, cx: &mut Cx, room_name_id: RoomNameId) {
        ...
        self.view.label(cx, ids!(card.title)).set_text(cx, "Create Room Bot");
        self.view.label(cx, ids!(card.body)).set_text(
            cx,
            &format!(
        ...
        self.view
            .text_input(cx, ids!(card.form.username_input))
            .set_text(cx, "");
        self.view
            .text_input(cx, ids!(card.form.display_name_input))
            .set_text(cx, "");
        self.view
            .text_input(cx, ids!(card.form.prompt_input))
            .set_text(cx, "");
        self.view.label(cx, ids!(card.status_label)).set_text(cx, "");
        self.view
            .button(cx, ids!(card.buttons.create_button))
            .set_enabled(cx, true);
        self.view
            .button(cx, ids!(card.buttons.cancel_button))
            .set_enabled(cx, true);
        self.view
            .button(cx, ids!(card.buttons.create_button))
            .reset_hover(cx);
