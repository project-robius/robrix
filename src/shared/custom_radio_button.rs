//! A radio button that can show a custom view, not just an icon/image.
...
live_design!{
    ...
    DrawCustomRadioButton = {{DrawCustomRadioButton}} {}
    pub CustomRadioButton = {{CustomRadioButton}} {}
    // pub CustomRadioButtonGroupBase = {{CustomRadioButtonGroup }} {}

    pub CustomRadioButtonFlat = <CustomRadioButton> {
    ...
    pub CustomRadioButton = <CustomRadioButtonFlat> {
        draw_bg: {
    ...
    pub CustomRadioButtonGradientY = <CustomRadioButton> {
        draw_bg: {
    ...
    pub CustomRadioButtonGradientX = <CustomRadioButtonGradientY> {
        draw_bg: {
    ...
    pub CustomRadioButtonCustom = <CustomRadioButton> {
        draw_bg: {
            fn pixel(self) -> vec4 {
                let sdf = Sdf2d::viewport(self.pos * self.rect_size)
                return sdf.result
    ...
    pub CustomRadioButtonFlatter = <CustomRadioButton> {
        draw_text: {
    ...
    pub CustomRadioButtonTabFlat = <CustomRadioButton> {
        height: Fit,
        label_walk: {
            margin: {
    ...
    pub CustomRadioButtonTab = <CustomRadioButtonTabFlat> {
        draw_bg: {
    ...
    pub CustomRadioButtonTabGradientY = <CustomRadioButtonTab> {
        draw_bg: {
    ...
    pub CustomRadioButtonTabGradientX = <CustomRadioButtonTabGradientY> {
        draw_bg: {
            gradient_border_horizontal: 1.0; 
            gradient_fill_horizontal: 1.0; 
        }
    ...
    pub CustomRadioButtonGroupTab = <CustomRadioButtonTab> {
        height: Fit,
...
#[repr(C)]
pub struct DrawCustomRadioButton {
...
pub struct CustomRadioButtonGroup {
    #[deref] frame: View
}

#[derive(Live, LiveHook, Widget)]
pub struct CustomRadioButton {
    #[redraw] #[live] draw_bg: DrawCustomRadioButton,
...
pub enum CustomRadioButtonAction {
    Clicked,
    None
}


impl CustomRadioButtonGroup {
    pub fn draw_walk(&mut self, _cx: &mut Cx2d, _walk: Walk) {}
}

impl CustomRadioButton {
    pub fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) {
        ...
        match self.media {
            ...
            MediaType::CustomView => {
                let _ = self.custom_view.draw_walk(cx, scope, walk);
            }
            MediaType::None => {}
        }
...
impl Widget for CustomRadioButtonGroup {
...
impl Widget for CustomRadioButton {
        ...
        match event.hits(cx, self.draw_bg.area()) {
            ...
            Hit::FingerUp(_fe) => {
                ...
                if self.animator_in_state(cx, ids!(active.off)) {
                    ...
                    cx.widget_action(uid, &scope.path, CustomRadioButtonAction::Clicked);
                } else {
    ...
    pub fn select(&self, cx: &mut Cx, scope: &mut Scope){
        if let Some(mut inner) = self.borrow_mut(){
            if inner.animator_in_state(cx, ids!(active.off)) {
                ...
                cx.widget_action(inner.widget_uid(), &scope.path, CustomRadioButtonAction::Clicked);
            }
        }
    }
}

impl CustomRadioButtonSet{
    pub fn selected(&self, cx: &mut Cx, actions: &Actions)->Option<usize>{
        for action in actions{
            if let Some(action) = action.as_widget_action(){
                match action.cast(){
                    CustomRadioButtonAction::Clicked => if let Some(index) = self.0.iter().position(|v| action.widget_uid == v.widget_uid()){
                        for (i, item) in self.0.iter().enumerate(){
                            if i != index{
                                CustomRadioButtonRef(item.clone()).unselect(cx);
                            }
                        }
                        return Some(index);
                    }
