...
impl WidgetMatchEvent for PopupList {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, _scope: &mut Scope) {
        ...
        for (i, (view, _data)) in self.popups.iter().enumerate() {
            ...
            actions.iter()
                .map(|action| Some(action.as_widget_action().widget_uid_eq(widget_uid).cast::<RobrixPopupNotificationAction>()))
                .filter(|action| matches!(action, Some(RobrixPopupNotificationAction::Ended)))
