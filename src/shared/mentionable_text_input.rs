...
use matrix_sdk::ruma::{events::room::message::RoomMessageEventContent, OwnedRoomId, OwnedUserId};
...
impl MentionableTextInput {
    ...
    fn on_user_selected(&mut self, cx: &mut Cx, _scope: &mut Scope, selected: WidgetRef) {
        ...
        if let Some(start_idx) = self.current_mention_start_index {
            ...
            if is_room_mention {
                ...
                // User selected a specific user - insert markdown link directly into text
                ...
                // Build the markdown link format: [displayname](https://matrix.to/#/@user:server)
                let mention_link = format!("[{}]({}) ", username, user_id.matrix_to_uri());

                // Replace the @ and search text with the markdown link
                let new_text = utils::safe_replace_by_byte_indices(
                    &current_text,
                    start_idx,
                    head,
                ...
                // Position cursor after the inserted mention link
                let new_pos = start_idx + mention_link.len();
                text_input_ref.set_cursor(cx, Cursor { index: new_pos, prefer_next_row: false }, false);
            }
        }
    ...
    fn handle_text_change(&mut self, cx: &mut Cx, scope: &mut Scope, text: String) {
        log!("[MentionableTextInput {:p}] handle_text_change called, text: '{}'", self as *const _, text);
...
impl MentionableTextInputRef {
    ...
    /// Clears all mention tracking state
    pub fn clear_pills(&self, _cx: &mut Cx) {
        if let Some(mut inner) = self.borrow_mut() {
    ...
    /// Returns true if there are any tracked mentions
    /// Note: This now checks possible_mentions which tracks mentions added during the session
    pub fn has_pills(&self) -> bool {
        self.borrow().is_some_and(|inner| !inner.possible_mentions.is_empty() || inner.possible_room_mention)
    }
